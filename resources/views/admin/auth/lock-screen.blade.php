...
<body class="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 min-h-screen font-sans">
    
    {{-- ✅ Alpine component with INLINE data - no registration needed, no timing issues --}}
    <div x-data="{
        currentTime: '',
        currentDate: '',
        showPinModal: false,
        ...
        selectEmployee(id, name, avatar) {
            console.log('selectEmployee called:', id, name);
            this.selectedId = id;
    ...
    }" class="fixed top-0 left-0 w-full h-full z-[99999] bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex flex-col items-center justify-center p-6 overflow-hidden">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 max-w-4xl">
            ...
            <button type="button"
                    @click="selectEmployee({{ $employee->id }}, '{{ $employee->name }}', '{{ $employee->avatar_url }}')"
